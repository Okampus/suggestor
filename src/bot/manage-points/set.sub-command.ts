...
@SubCommand({ name: 'set', description: messagesConfig.managePointsCommand.set.description })
export class SetSubCommand implements DiscordTransformedCommand<UpdatePointsDto> {
  constructor(
  ...
  public async handler(
    ...
    userPoint.points = Math.max(updatePointsDto.amount, 0);
    await this.userPointRepository.flush();

    await interaction.reply({
      content: pupa(messagesConfig.managePointsCommand.set.success, { ...userPoint }),
